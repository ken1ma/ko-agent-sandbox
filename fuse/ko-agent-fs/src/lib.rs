...
//!
//! `docs/git-metadata.md` is the security analysis this code transcribes. `policy` alone decides
//! *whether* an operation is allowed, and imports nothing; `fs`, `inode` and `guard` are the
//! plumbing that decides *where* it is (`docs/architecture.md`, "What stays out of the audited
//! core").
